...
public class galleryObjGenerator : MonoBehaviour {
	...
	public IEnumerator Start () {
		...
		for (int i=1; i<=fcount; i++) { // Create object number of fcount 
			...
			//yield return new WaitForSeconds(0.00001f); // Start delay
			...
			yield return new WaitForSeconds(0.001f); // End delay
			
		}

	}

	private void destoryPictureObjs(){
		Debug.Log ("destory");
		for(int i = 1 ; i <= limit; i++)
		{
			GameObject picture = GameObject.Find("picture"+i.ToString());
